...
const Carousel = ({pois, imageData}: {pois: Poi[], imageData: string[]}) => {
    ...
    const handleIncreasePoiIndex = () => (activePoiId === pois.length - 1) ? null : setActivePoiId(activePoiId + 1);
    ...
    return (
        <>
            ...
            <div className="flex flex-row justify-evenly w-2/5 mt-4">
                {pois.map((poi: Poi, index: number) => <CarouselDot key={poi.title} index={index} activePoiId={activePoiId} setActivePoiId={setActivePoiId} /> )}
            </div>
        </>
    );
};
